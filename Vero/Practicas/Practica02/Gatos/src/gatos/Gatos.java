...
public class Gatos extends PApplet {
    ...
    int profundidad = 0;       // Profundidad a la que se ha expandido el Ã¡rbol.
    int diametroMaximo = 1;    // Mayor nÃºmero de estados expandidos a la misma profundidad.    
    
    int anchoImagen;
    int altoImagen;
    int anchoGato, altoGato;           // Dimensiones en pixeles del dibujo de cada gato.
    boolean modificaVentana = false;   // Indica cuando las dimensiones del Ã¡rbol se han incrementado.
    boolean genera = false;            // Bandera para solicitar la expansiÃ³n del siguiente nivel.
    
    Gato gatoRaiz;                     // Estado inicial
    LinkedList<Gato> listaAbierta = new LinkedList();  // Nodos en el nivel mÃ¡s profundo que no han sido expandidos.
    ...
    public void draw() {
        ...
        if (modificaVentana){
          ...
          System.out.println("Diámetro mÃ¡ximo " + diametroMaximo + ". Nodos en el Ãºltimo nivel: " + listaAbierta.size());          
        ...
        while(!listaGatos.isEmpty()) {
          ...
          int hijosIzquierda = 0; // Descendientes que irÃ¡n siendo dibujados.
          for(int i = 0; i < numGatos; i++){
            ...
            if(actual.sucesores != null){
              ...
              //dibujaGato(actual, anchoGato * hijosIzquierda, altoGato * profundidadActual);
              //int posicionPadre = hijosIzquierda;
              for(int h = 0; h < numHijos; h++){
                //line((int)((0.5+posicionPadre)*anchoGato), altoGato * profundidadActual + 4 * tamanioMosaico,
        ...
        text("DiÃ¡metro: " + diametroMaximo, 5, altoImagen + 24);        
    ...
    /**
    * FunciÃ³n encargada de generar el siguiente nivel en BFS.
    ...
    */
    protected void generaSiguienteNivel(){
        // Si ya se alcanzÃ³ la profundidad mÃ¡xima (9 jugadas) no hace nada.
        ...
        System.out.print("\nGanaX: " + ganaX);
        System.out.print(" GanaO: " + ganaO );
        System.out.print(" Empates: " + empates);
          
        
        // Actualiza variables de estado para saber en quÃ© punto se encuentra el proceso
